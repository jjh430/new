...
// 음식명 → 위키백과(Wikipedia) 검색어 매핑 (정확도 최적화)
const foodMapping = {
    // 한식
    ...
    '치킨': '프라이드치킨', '양념치킨': '양념치킨', '연어회': '연어',
    '수육': '수육', '감자탕': '감자탕', '순대국': '순대',
    '콩나물국밥': '콩나물국밥', '추어탕': '추어탕', '고등어조림': '고등어조림',
    '갈치조림': '갈치조림', '오리구이': '오리고기', '매운탕': '매운탕',
    ...
    '케밥': '케밥', '타코': '타코', '부리토': '부리토',
    '푸팟퐁커리': '뿌 빳 퐁 까리', '카오만가이': '카우 만 까이',

    // 분식/간식
    '와플': '와플', '팬케이크': '팬케이크', '핫도그': '핫도그',
    '붕어빵': '붕어빵', '호떡': '호떡', '튀김': '튀김',
    '도넛': '도넛', '샌드위치': '샌드위치', '계란빵': '계란빵'
};
...
async function showFoodImage(foodName) {
    ...
    try {
        ...
        // 위키백과 API로 한국어 이미지 검색
        const res = await fetch(
            `https://ko.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(searchTerm)}&prop=pageimages&format=json&pithumbsize=600&origin=*`
        );
